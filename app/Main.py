...
st.markdown("""
        ...
        удаления каких-либо аномальных значений (выбросов). Многие из распределений, как, например, pH (изображена ниже), оказались
...
st.markdown("#### гистограмма pH")
fig, ax = plt.subplots()
ax.hist(data["pH"], bins=20)
st.pyplot(fig)
